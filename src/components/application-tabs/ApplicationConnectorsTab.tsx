...
export function ApplicationConnectorsTab({
  ...
  const { data: appConnectors = [], isLoading } = useQuery<Connector[]>({
    queryKey: ["connectors", organizationId, applicationId],
    queryFn: () => connectorsApi.list(organizationId!, applicationId!),
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {appConnectors.map((connector) => (
            <Card
              ...
              data-testid={`card-app-connector-${connector.id}`}
              onClick={() => {
                if (applicationId) {
                  setLocation(
                    `/connectors/${applicationId}/${connector.id}`
                  );
                }
              }}
            >
              <CardContent className="p-6">
                <div className="flex items-center gap-6">
                  <div className="w-24 h-24 flex items-center justify-center bg-gray-100 dark:bg-gray-100 rounded-lg flex-shrink-0">
                    <img
                      src={getConnectorIconUrl(
                        connector.name,
                        connector.driver || connector.template_id || ""
                      )}
                      alt={connector.name}
