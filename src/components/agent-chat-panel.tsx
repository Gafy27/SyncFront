...
function ToolGroup({ messages }: { messages: AgentMessage[] }) {
  ...
  const names = messages.map((m) => m.tool_name).filter(Boolean)

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-0.5">
      <button
        onClick={() => setOpen((v) => !v)}
        ...
        {open && (
          ...
          >
            <div className="mt-1.5 space-y-0.5">
              {names.map((name, i) => (
                <p key={i} className="text-xs text-muted-foreground/50 pl-0.5">
                  {name}
                </p>
              ))}
            </div>
                ...
                >
                  {(() => {
                    ...
                    while (i < chat.messages.length) {
                      if (isTool(chat.messages[i].role)) {
                        const toolMsgs: AgentMessage[] = []
                        while (i < chat.messages.length && isTool(chat.messages[i].role)) {
                          toolMsgs.push(chat.messages[i++])
                        }
                        groups.push(<ToolGroup key={`tools-${i}`} messages={toolMsgs} />)
                      } else if (chat.messages[i].role === "user" || chat.messages[i].role === "assistant") {
                        groups.push(<MessageBubble key={i} message={chat.messages[i++]} />)
                      } else {
                        i++ // skip unknown roles silently
                      }
                    }
                    return groups
                  })()}
                  ...
                  {/* Thinking */}
                  <AnimatePresence>
                    {chat.isWaiting && <ThinkingIndicator key="thinking" />}
                  </AnimatePresence>
